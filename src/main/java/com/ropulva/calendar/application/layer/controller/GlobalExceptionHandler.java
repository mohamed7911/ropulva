...
public class GlobalExceptionHandler {
    ...
    public BaseDTO<Map<String, List<String>>> handleValidationErrors(MethodArgumentNotValidException ex) {
        List<String> errors = ex.getBindingResult().getFieldErrors()
                .stream().map(FieldError::getDefaultMessage).collect(Collectors.toList());
        return new BaseDTO<>(ResponseStatusEnum.FAILED.name(), getErrorsMap(errors), false);
    }
